...
class ScreenshotDetectionPlugin(Star):
    ...
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        self._target_umo: str | None = self._normalize_umo(target_umo) if target_umo else None
        ...
        # 截图分析提示词
        self._analysis_prompt: str = config.get("analysis_prompt", 
        ...
        if self._max_screenshots < 1:
            self._max_screenshots = 1
    ...
    async def initialize(self):
        ...
        if self._auto_start and self._target_umo:
            ...
            logger.info(f"截图任务已启动，间隔 {self._interval} 秒，目标 {self._target_umo}")
        ...
        while self._running:
            try:
                ...
                if self._is_quiet_time():
                    logger.info(f"当前在免打扰时段 ({self._quiet_start}-{self._quiet_end})，跳过截图")
                    continue
                ...
                try:
                    ...
                    success = await self.context.send_message(self._target_umo, text_chain)
    ...
    async def _analyze_screenshot(self, image_bytes: bytes, event: AstrMessageEvent) -> str:
        """使用当前会话人格分析截图"""
        try:
            persona = None
            if event:
                try:
                    persona_id = self.context.get_curr_persona_id(event.unified_msg_origin)
                    if persona_id:
    ...
    async def start_screenshot(self, event: AstrMessageEvent):
        ...
        yield event.plain_result(f"截图检测已启动\n间隔时间: {interval_text}{quiet_info}")
    ...
    async def set_interval(self, event: AstrMessageEvent):
        ...
        args = event.message_str.split()
        if len(args) < 2:
            yield event.plain_result(f"当前间隔: {self._format_interval(self._interval)}")
            return
        ...
        try:
            ...
            self._save_config()
            yield event.plain_result(f"截图间隔已设置为: {self._format_interval(self._interval)}")
        except ValueError:
    ...
    async def test_send(self, event: AstrMessageEvent):
        ...
        try:
            ...
            yield event.plain_result(f"发送异常: {str(e)}")
    ...
    async def screenshot_status(self, event: AstrMessageEvent):
        ...
        last_time = datetime.fromtimestamp(self._last_screenshot_time).strftime("%Y-%m-%d %H:%M:%S") if self._last_screenshot_time else "无"
        quiet_info = "未设置"
        if self._quiet_start is not None and self._quiet_end is not None:
